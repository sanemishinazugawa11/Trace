...
export default function ChatScreen() {
  ...
  const [messages, setMessages] = useState(() => {
    if (params.pastMessages) {
      try { return JSON.parse(params.pastMessages as string); } 
      catch (e) { console.error('Failed to parse past messages', e); }
    }
    return [{ id: '1', role: 'ai', text: 'Trace system active. Paste your error stack or code snippet below.' }];
  });
  ...
  return (
    ...
    >
        ...
        {messages.map((msg: any, index: number) => {
          ...
          return (
            <View key={key} style={[styles.messageWrapper, isUser ? styles.messageWrapperUser : styles.messageWrapperAi]}>
              ...
              {!isUser && (
                <TouchableOpacity style={styles.copyButton} onPress={() => copyToClipboard(msg.text)} activeOpacity={0.6}>
                  <Ionicons name="copy-outline" size={12} color={subText} />
                  <Text style={[styles.copyText, { color: subText }]}>Copy</Text>
                </TouchableOpacity>
              )}
            </View>
          );
      ...
      <View style={[styles.inputBar, { backgroundColor: inputBg, borderColor: borderColor }]}>
        <TextInput
...
const styles = StyleSheet.create({
  container: { flex: 1 }, 
  title: { fontSize: 34, fontWeight: '700', letterSpacing: -0.5, marginBottom: 16, paddingHorizontal: 20 },
  ...
  copyButton: { flexDirection: 'row', alignItems: 'center', marginTop: 5, marginLeft: 6 },
